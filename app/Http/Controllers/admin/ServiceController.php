...
class ServiceController extends Controller
{
    public function add(Request $request, $id = Null)
    {
        ...
        $getdata = Service::find($decrypted_id);
        $category = ServiceCategory::where('status', 1)->get();
        if ($id != "") {
            $saveurl = url('admin/service/save/' . $id);
            $button = 'Update';
            $page_title = 'Update service category';
        } else {
            $saveurl = url('admin/service/save');
            $button = 'Add';
    ...
    public function save(Request $request, $id = NUll)
    {
        if (!empty($id)) {
            $decrypted_id  = get_decrypted_value($id, true);
            $data          = Service::find($decrypted_id);
            $success_msg   = 'Service Category Updated Successfully.';
            $nameValidator = 'required|unique:services,title,' . $decrypted_id . ',id,status,1';
        } else {
            $data          = new Service;
            $success_msg   = 'Service Category Added Successfully.';
            $nameValidator = 'required|unique:services,title,3,status';
        }
        ...
        if ($Validatior->fails()) {
            ...
            try {
                if ($request['image'] != "") {
                    ...
                    // }
                    $request->file('image')->move("uploads/services", $name);
        ...
        return redirect()->route('service')->withSuccess($success_msg);
    }
    ...
    public function index()
    {
        ...
        return view('admin.Service.view')->with($data);
    }

    public function anydata(Request $request)
    {
        $anydata = [];
        $anydata = Service::orderBy('id', 'DESC')->where('status', '<', 3)->where(function ($query) use ($request) {
        ...
        return Datatables::of($anydata)
            ...
            ->addColumn('action', function ($anydata) {
                ...
                $action = '<a href="' . url('/admin/service/add/' . $encrypted_id) . '"><i class="fas fa-edit" data-original-title="Edit" data-toggle="tooltip" data-placement="bottom"></i></a> &nbsp;&nbsp;  
    ...
    public function delete(Request $request)
    {
        ...
        if ($data) {
            ...
            $return_arr = array(
                'status' => 'success',
                'message' => 'Service Deleted Sussessfully!',
            );
