...
class BlogController extends Controller
{
    public function add(Request $request, $id = Null)
    {
        ...
        $getdata = Blog::find($decrypted_id);
        if ($id != "") {
            $saveurl = url('admin/blog/save/' . $id);
            $button = 'Update';
            $page_title = 'Update Blog';
        } else {
            $saveurl = url('admin/blog/save');
            $button = 'Add';
            $page_title = 'Add Blog';
        }
        $data = array(
    ...
    public function save(Request $request, $id = NUll)
    {
        if (!empty($id)) {
            $decrypted_id  = get_decrypted_value($id, true);
            $data          = Blog::find($decrypted_id);
            $success_msg   = 'Blog Updated Successfully.';
            $nameValidator = 'required|unique:blogs,title,' . $decrypted_id . ',id,status,1';
        } else {
            $data          = new Blog;
            $success_msg   = 'Blog Added Successfully.';
            $nameValidator = 'required|unique:blogs,title,3,status';
        }
        ...
        if ($Validatior->fails()) {
            ...
            try {
                if ($request['image'] != "") {
                    ...
                    $request->file('image')->move("uploads/blog", $name);
                    $data->image = 'uploads/blog/' . $name;
                }
        ...
        return redirect()->route('blog')->withSuccess($success_msg);
    }
    ...
    public function index()
    {
        ...
        return view('admin.blog.view')->with($data);
    }

    public function anydata(Request $request)
    {
        $anydata = [];
        $anydata = Blog::orderBy('id', 'DESC')->where('status', '<', 3)->where(function ($query) use ($request) {
        ...
        return Datatables::of($anydata)
            ...
            ->addColumn('action', function ($anydata) {
                ...
                $action = '<a href="' . url('/admin/blog/add/' . $encrypted_id) . '"><i class="fas fa-edit" title="Edit" data-toggle="tooltip" data-placement="bottom"></i></a> &nbsp;&nbsp;  
    ...
    public function delete(Request $request)
    {
        ...
        if ($data) {
            ...
            $return_arr = array(
                'status' => 'success',
                'message' => 'Blog Deleted Sussessfully!',
            );
