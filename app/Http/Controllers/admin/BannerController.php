...
class BannerController extends Controller
{
            ...
            try {
                if ($request['image'] != "") {
                 ...
                 if ($request['mobileimage'] != "") {
                    $file1 = $request->file('mobileimage');
                    $name1 = rand(11111, 99999).date('d-m-y'). '.' . $file1->getClientOriginalExtension();
                    ...
                    $request->file('mobileimage')->move("uploads/banner", $name1);
                    $data->mobileimage = 'uploads/banner/' . $name1;
                }
                ...
                $data->url_title       = $request['url_title'];
                 $data->slug        = isset($request['slug'])?$request['slug']:Str::slug($request['title']);
    ...
    public function anydata(Request $request)
    {
        ...
        return Datatables::of($anydata)
            ...
            ->addColumn('mobileimage', function ($anydata) {
                if ($anydata['mobileimage'] != "" || file_exists($anydata['mobileimage'])) {
                    $img = url($anydata['mobileimage']);
                } else {
                    $img = url('public/noimage.png');
                }
                return '<img style="border-radius: 50%;" alt="image" src=' . $img . ' width="35" height="35px">';
            ...
            ->rawColumns(['status', 'action', 'image','mobileimage'])
