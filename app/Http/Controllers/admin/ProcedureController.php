...
class ProcedureController extends Controller
{
    public function add(Request $request, $id = Null)
    {
        ...
        $getdata = Procedure::find($decrypted_id);
        if ($id != "") {
            $saveurl = url('admin/procedure/save/' . $id);
            $button = 'Update';
            $page_title = 'Update service category';
        } else {
            $saveurl = url('admin/procedure/save');
            $button = 'Add';
        ...
        return view('admin.Procedure.add')->with($data);
    }
    public function save(Request $request, $id = NUll)
    {
        if (!empty($id)) {
            $decrypted_id  = get_decrypted_value($id, true);
            $data          = Procedure::find($decrypted_id);
            $success_msg   = 'Procedure Updated Successfully.';
            $nameValidator = 'required|unique:procedures,title,' . $decrypted_id . ',id,status,1';
        } else {
            $data          = new Procedure;
            $success_msg   = 'Procedure Added Successfully.';
            $nameValidator = 'required|unique:procedures,title,3,status';
        }
        ...
        return redirect()->route('procedure')->withSuccess($success_msg);
    }
    ...
    public function index()
    {
        $data = array(
            ...
            'page_title' => 'View Procedure',
        );
        return view('admin.Procedure.view')->with($data);
    }

    public function anydata(Request $request)
    {
        $anydata = [];
        $anydata = Procedure::orderBy('id', 'DESC')->where('status', '<', 3)->where(function ($query) use ($request) {
        ...
        return Datatables::of($anydata)
            ...
            ->addColumn('action', function ($anydata) {
                ...
                $action = '<a href="' . url('/admin/procedure/add/' . $encrypted_id) . '"><i class="fas fa-edit" data-original-title="Edit" data-toggle="tooltip" data-placement="bottom"></i></a> &nbsp;&nbsp;  
    ...
    public function delete(Request $request)
    {
        ...
        if ($data) {
            ...
            $return_arr = array(
                'status' => 'success',
                'message' => 'Procedure Deleted Sussessfully!',
            );
